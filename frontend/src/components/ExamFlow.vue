...
import { ChevronLeft, ChevronRight, CheckCircle2, PlayCircle, ClipboardCheck, LockKeyhole, RotateCcw, LogOut } from 'lucide-vue-next';
import { useAuth } from '../lib/auth';

type Option = { id: number; letter: string; text: string };
type Question = { id: number; number: number; text: string; options: Option[] };

const { user, fullName, apiFetch, logout } = useAuth();

const stage = ref<'intro' | 'exam' | 'result'>('intro');
...
const result = ref({ score: 0, correct: 0, total: 0 });
...
const canNext = computed(() => q.value && answers.value[q.value.id] != null);
...
// Al terminar el video, arranca la evaluación directamente con los datos
// del usuario ya autenticado (ya no se pide nombre en pantalla).
async function videoEnded() {
  videoDone.value = true;
}

async function startExam() {
  if (!videoDone.value || loading.value) return;
  loading.value = true;

  const startRes = await apiFetch('/api/evaluations/start', { method: 'POST' });
  const startData = await startRes.json();
  evaluationId.value = startData.evaluation_id;

  await apiFetch(`/api/evaluations/${evaluationId.value}/video-completed`, { method: 'POST' });
...
async function finish() {
  ...
  const payload = {
    answers: questions.value.map((x) => ({
      question_id: x.id,
      option_id: answers.value[x.id],
    })),
  };

  const r = await apiFetch(`/api/evaluations/${evaluationId.value}/finish`, {
    method: 'POST',
...
<template>
  ...
  <div class="min-h-screen bg-[#f4f4f3] text-[#575756]">
    ...
    <main class="mx-auto max-w-6xl px-5 py-8 md:py-6">
      <section v-if="stage === 'intro'" class="flex flex-col items-center">
        <div class="mx-auto max-w-4xl text-center">
          <span class="inline-flex items-center gap-2 rounded-full bg-[#dadada]/60 px-3 py-1 text-xs font-semibold uppercase tracking-wider">
          ...
          <p class="mt-5 mx-auto max-w-3xl text-base leading-7 text-[#878787] md:text-lg">
            Visualiza el material completo antes de acceder a la evaluación del <br /><b>Módulo 1: Nociones básicas de derecho electoral.</b>
          </p>
        ...
        <div class="mt-8 w-full max-w-6xl">
          <div class="mx-auto max-w-5xl rounded-[32px] border border-[#dadada] bg-white p-3 shadow-xl shadow-black/5 md:p-4">
            <div class="overflow-hidden rounded-[26px] bg-[#575756]">
              <video class="block aspect-video w-full object-cover" controls playsinline preload="metadata" @ended="videoEnded">
                <source src="/curso-modulo1.mp4" type="video/mp4" />Tu navegador no soporta video HTML5.
              </video>
            </div>
            <div class="flex flex-col items-center justify-between gap-4 px-2 py-4 text-center sm:flex-row sm:text-left">
              <div>
      ...
      <section v-else-if="stage === 'exam'" class="mx-auto max-w-4xl">
        ...
        <div class="h-2 overflow-hidden rounded-full bg-[#dadada]">
          <div class="h-full rounded-full bg-[#575756] transition-all duration-500" :style="{ width: progress + '%' }"></div>
        </div>
        <div class="mt-8 rounded-3xl border border-[#dadada] bg-white p-6 shadow-xl shadow-black/5 md:p-10">
          <Transition name="fade" mode="out-in">
            <div :key="q.id">
              <div class="flex items-start gap-4">
                <span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#575756] text-sm font-black text-white">
                  {{ q.number }}
                </span>
              ...
              <div class="mt-8 space-y-3">
                <button
                  ...
                  :class="answers[q.id] === o.id ? 'border-[#575756] bg-[#f0f0ef] shadow-sm' : 'border-[#dadada] hover:border-[#878787] hover:bg-[#fafafa]'"
                >
                  <span
                  ...
                  <span class="pt-1 text-sm leading-6 md:text-base">{{ o.text }}</span>
                  <CheckCircle2 v-if="answers[q.id] === o.id" class="ml-auto mt-1 shrink-0" :size="20" />
                </button>
              </div>
            </div>
          </Transition>
          <div class="mt-9 flex items-center justify-between border-t border-[#dadada] pt-5">
            <button @click="previous" :disabled="current === 0" class="inline-flex items-center gap-2 rounded-xl px-4 py-3 text-sm font-bold disabled:opacity-30">
            ...
            <button
              ...
              class="inline-flex items-center gap-2 rounded-xl bg-[#575756] px-5 py-3 text-sm font-bold text-white disabled:opacity-30"
            >
              {{ loading ? 'Guardando…' : 'Finalizar evaluación' }} <CheckCircle2 :size="18" />
            </button>
          </div>
        </div>
      ...
      <section v-else class="mx-auto max-w-3xl">
        <div class="overflow-hidden rounded-[32px] border border-[#dadada] bg-white text-center shadow-xl shadow-black/5">
          ...
          <div class="p-8 md:p-10">
            ...
            <p class="mt-4 text-base leading-7 text-[#878787]">
              Gracias por completar el módulo de capacitación. Tu participación ha sido registrada de forma exitosa en la plataforma institucional.
            </p>
            <button @click="restart" class="mt-8 inline-flex items-center gap-2 rounded-xl bg-[#575756] px-5 py-3 text-sm font-bold text-white hover:bg-[#454544]">
    ...
    <footer class="mx-auto max-w-6xl px-5 pb-8 text-center text-xs text-[#878787]">
      Fiscalía General de Justicia del Estado de Tamaulipas · Fiscalía Especializada en Delitos Electorales
    </footer>
  </div>
</template>